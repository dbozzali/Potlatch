...
public class SecuredRestBuilder extends RestAdapter.Builder {
		...
		public void intercept(RequestFacade request) {
			...
			if (!this.loggedIn) {
				try {
					...
					to.addField("username", this.username);
					to.addField("password", this.password);
					
					// Add the client ID and client secret to the body of the request.
					to.addField("client_id", this.clientId);
					to.addField("client_secret", this.clientSecret);
					...
					// above.
					String base64Auth =
						BaseEncoding.base64().encode(new String(this.clientId + ":" + this.clientSecret).getBytes());
					...
					Request req = new Request("POST", this.tokenIssuingEndpoint, headers, to);
					
					// Request the password grant.
					Response resp = this.client.execute(req);
	...
	public RestAdapter build() {
		...
		OAuthHandler handler =
			new OAuthHandler(this.client, this.loginUrl, this.username, this.password, this.clientId, this.clientSecret);
